...
//		// We map the lowercase filename (without extension usually) to the file pointer.
//		fileMap := make(map[string]*File)
//		for _, file := range files {
//			// Ensure we are matching against the file name logic consistent with your links
...
//			if len(sourceFile.Links) == 0 {
//				continue
//			}
//
//			for _, rawLink := range sourceFile.Links {
//				// --- A. CLEAN THE LINK ---
//				// rawLink is something like "[[My Page#Heading|Alias]]"
//
//				// 1. Remove surrounding brackets [[ and ]]
//				cleanLink := strings.TrimPrefix(rawLink, "[[")
//				cleanLink = strings.TrimSuffix(cleanLink, "]]")
//
//				// 2. Remove Alias (if exists) -> "My Page#Heading"
//				if idx := strings.Index(cleanLink, "|"); idx != -1 {
//					cleanLink = cleanLink[:idx]
//				}
//
//				// 3. Remove Anchor/Header (if exists) -> "My Page"
//				if idx := strings.Index(cleanLink, "#"); idx != -1 {
//					cleanLink = cleanLink[:idx]
//				}
//
//				// 4. Normalize for Lookup
//				// Handle paths like "Folder/Note" -> just "Note" (if your map is flat)
//				// Remove this block if your fileMap keys include the full folder path.
...
//					// Prevent self-linking in backlinks list
//					if targetFile.Path == sourceFile.Path {
//						continue
//					}
//
//					// Format the backlink as requested: [[/paths/webpath]]
//					// assuming sourceFile.WebPath is the string you want inside.
