...
window.setupRightSidebarInteraction = function () {
    ...
    toggleBtns.forEach(btn => {
        ...
        newBtn.addEventListener('click', () => {
            sidebar.classList.toggle('collapsed');
            localStorage.setItem('right-sidebar-collapsed', sidebar.classList.contains('collapsed'));
        });
    });
};
...
window.setupMobileAutoClose = function () {
    document.body.addEventListener('click', (e) => {
        ...
        const isGraphNode = (['circle', 'text'].includes(e.target.tagName.toLowerCase()) &&
            (e.target.closest('#global-graph-container') || e.target.closest('#local-graph-container')));
...
window.toggleGraphExpand = function () {
    ...
    if (icon) {
        icon.setAttribute('data-lucide', wrapper.classList.contains('expanded') ? 'minimize-2' : 'maximize-2');
        if (window.lucide) window.lucide.createIcons();
    }
