...
window.JsonCanvasRenderer = class JsonCanvasRenderer {
    ...
    if (this.edgesLayer) {
        ...
        this.state = { scale: 1, panX: 0, panY: 0, isDragging: false, lastMouseX: 0, lastMouseY: 0 };
        this.data = null;
    ...
    render() {
        ...
        if (this.data.edges) this.data.edges.forEach(edge => this.createEdge(edge));
    }
    ...
    createNode(nodeData) {
        ...
        switch (nodeData.type) {
            ...
            case 'file':
                ...
                if (nodeData.isImage || /\.(png|jpg|jpeg|gif|svg|webp)$/i.test(nodeData.file)) {
                    // IMAGE NODE
    ...
    getAnchorPoint(node, side) {
        ...
        if (!side) return { x, y };
        switch (side) {
            case 'top': y = node.y; break;
            case 'bottom': y = node.y + node.height; break;
            case 'left': x = node.x; break;
            case 'right': x = node.x + node.width; break;
        }
        return { x, y };
    }
    ...
    getControlPoint(point, side) {
        ...
        if (!side) return { x, y };
        switch (side) {
            case 'top': y -= curvature; break;
            case 'bottom': y += curvature; break;
            case 'left': x -= curvature; break;
            case 'right': x += curvature; break;
        }
        return { x, y };
    }
    ...
    zoomIn() { this.state.scale = Math.min(this.state.scale + 0.2, 5); this.updateTransform(); }
    zoomOut() { this.state.scale = Math.max(0.1, this.state.scale - 0.2); this.updateTransform(); }
    resetView() { this.centerCanvas(); }
    ...
    centerCanvas() {
        ...
        this.state.panX = (viewWidth - contentWidth * this.state.scale) / 2 - (minX * this.state.scale);
        this.state.panY = (viewHeight - contentHeight * this.state.scale) / 2 - (minY * this.state.scale);
        this.updateTransform();
    }
};
